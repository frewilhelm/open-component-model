...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, err error) {
	...
	// Add verification signatures if any are passed
	verifications := component.GetVerifications()
	if len(verifications) != 0 {
		signatures := make(map[string]string, len(verifications))
		for _, signature := range verifications {
			switch {
			case signature.Value != "":
				signatures[signature.Signature] = signature.Value
			case signature.SecretRef.Name != "":
				secret := &corev1.Secret{}
				if err := r.Get(ctx, types.NamespacedName{
					Namespace: component.GetNamespace(),
					Name:      signature.SecretRef.Name,
				}, secret); err != nil {
					return ctrl.Result{}, fmt.Errorf("failed to get secret %q for signature %q: %w", signature.SecretRef.Name, signature.Signature, err)
				}

				signatures[signature.Signature] = string(secret.Data[signature.Signature])
			default:
				return ctrl.Result{}, fmt.Errorf("no value or secretRef provided for signature %q", signature.Signature)
			}
		}

		cacheBackedRepo.SigningRegistry = r.PluginManager.SigningRegistry
		cacheBackedRepo.Signatures = signatures
	}
